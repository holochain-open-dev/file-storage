...
import { TypeFormatFlags, SymbolFlags, isVariableDeclaration, isVariableStatement, isMethodDeclaration } from 'typescript';
import { toSimpleType, isSimpleType, typeToString, isAssignableToSimpleTypeKind } from 'ts-simple-type';
...
function getLibTypeWithName(name, _a) {
    ...
    var type = node == null ? undefined : toSimpleType(node, checker);
    if (type != null) {
...
function getJsDoc(node, ts, tagNames) {
    ...
    if (jsDocNode == null) {
        ...
        if (leadingComment != null) {
            ...
            return __assign(__assign({}, jsDoc), { tags: (_a = jsDoc.tags) === null || _a === void 0 ? void 0 : _a.filter(function (t) { return tagNames.includes(t.tag); }) });
        }
        return undefined;
    }
...
function discoverEvents(node, context) {
    ...
    if (ts.isNewExpression(node)) {
        ...
        if (EVENT_NAMES.includes(expression.getText()) && args && args.length >= 1) {
            var arg = args[0];
            var eventName = (_a = resolveNodeValue(arg, __assign(__assign({}, context), { strict: true }))) === null || _a === void 0 ? void 0 : _a.value;
...
function applyJsDocAttribute(feature, jsDoc, context) {
    ...
    if (attributeTag != null && feature.attrName == null) {
        ...
        var result = __assign(__assign({}, feature), { attrName: attributeTag.parsed().name || feature.propName, default: (_b = feature.default) !== null && _b !== void 0 ? _b : parsed_1.default });
...
function applyJsDocType(feature, jsDoc, context) {
    ...
    if (typeTag != null && feature.typeHint == null) {
        ...
        if (parsed_2.type != null && parsed_2.type.length > 0) {
            return __assign(__assign({}, feature), { typeHint: parsed_2.type, type: (_b = feature.type) !== null && _b !== void 0 ? _b : lazy(function () { return parseSimpleJsDocTypeExpression(parsed_2.type || "", context); }) });
        }
    }
    return feature;
}
...
function mergeJsDoc(leftJsDoc, rightJsDoc) {
    ...
    return __assign(__assign({}, (leftJsDoc !== null && leftJsDoc !== void 0 ? leftJsDoc : rightJsDoc)), { description: (_a = leftJsDoc.description) !== null && _a !== void 0 ? _a : rightJsDoc.description });
}
/**
 * Merges modifiers
 * @param leftModifiers
...
function mergeEvents(events) {
    return mergeNamedEntries(events, function (event) { return event.name; }, function (left, right) { return (__assign(__assign({}, left), { jsDoc: mergeJsDoc(left.jsDoc, right.jsDoc), type: function () { return (left.type != null ? left.type() : right.type != null ? right.type() : { kind: "ANY" }); }, typeHint: left.typeHint || right.typeHint })); });
}
...
function mergeMemberIntoMember(leftMember, rightMember, checker) {
    ...
    return __assign(__assign({}, leftMember), { attrName: (_a = leftMember.attrName) !== null && _a !== void 0 ? _a : rightMember.attrName, type: (function () {
            var _a, _b;
...
function analyzeComponentDeclaration(initialDeclarationNodes, baseContext, options) {
    ...
    var context = __assign(__assign({}, baseContext), { declarationNode: mainDeclarationNode, sourceFile: mainDeclarationNode.getSourceFile(), getDeclaration: function () { return baseDeclaration; } });
    ...
    try {
        for (var declarationNodes_1 = __values(declarationNodes), declarationNodes_1_1 = declarationNodes_1.next(); !declarationNodes_1_1.done; declarationNodes_1_1 = declarationNodes_1.next()) {
            ...
            featureCollections.push(discoverFeatures$1(node, __assign(__assign({}, context), { declarationNode: node, sourceFile: node.getSourceFile() })));
        }
    }
    catch (e_2_1) { e_2 = { error: e_2_1 }; }
    finally {
...
function discoverDefinitions$4(sourceFile, context, analyzeDeclaration) {
    ...
    return Array.from(definitionResults.entries()).map(function (_a) {
        ...
        return __assign(__assign({}, definition), { get declaration() {
            ...
            switch (exp.flags) {
                ...
                case SymbolFlags.Variable: {
                    ...
                    if (isVariableDeclaration(node)) {
                        ...
                        var variableStatement = findParent(node, isVariableStatement) || node;
...
function getEventDocsFromDeclaration(declaration, context) {
    return filterVisibility(context.config.visibility, declaration.events).map(function (event) {
        ...
        var simpleType = isSimpleType(type) ? type : toSimpleType(type, context.checker);
...
function componentMemberToVscodeAttr(member, checker) {
    ...
    return __assign({ name: member.attrName, description: formatMetadata(formatEntryRow(member.attrName, member.jsDoc, member.typeHint || ((_a = member.type) === null || _a === void 0 ? void 0 : _a.call(member)), checker), {
...
function typeToVscodeValuePart(type, checker) {
    var simpleType = isSimpleType(type) ? type : toSimpleType(type, checker);
...
export { ALL_COMPONENT_FEATURES as A, CustomElementFlavor as C, DEFAULT_FEATURE_COLLECTION_CACHE as D, JsDocFlavor as J, VERSION as V, __assign as _, arrayDefined as a, analyzeSourceFile as b, DEFAULT_COMPONENT_DECLARATION_CACHE as c, analyzeComponentDeclaration as d, getMixinHeritageClauses as e, getExtendsHeritageClauses as f, getSuperclassHeritageClause as g, getMixinHeritageClausesInChain as h, getExtendsHeritageClausesInChain as i, getJsDoc as j, getJsDocType as k, __awaiter as l, makeContextFromConfig as m, __generator as n, __values as o, parseSimpleJsDocTypeExpression as p, arrayFlat as q, __read as r, stripTypescriptValues as s, transformAnalyzerResult as t, visitAllHeritageClauses as v };
