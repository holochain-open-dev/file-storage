...
class DefaultRenderer {
    constructor(tasks, options, renderHook$) {
        ...
        this.figures = !is_unicode_supported_1.isUnicodeSupported() ? figures : figures.main;
            ...
            if (task.isEnabled()) {
                ...
                if (task.hasTitle()) {
                    if (!(tasks.some((task) => task.hasFailed()) && !task.hasFailed() && task.options.exitOnError !== false && !(task.isCompleted() || task.isSkipped()))) {
                        ...
                        output = [...output, this.formatString(task.title, colorette_1.default.red(figures.main.squareSmallFilled), level)];
                    }
                }
    ...
    getSymbol(task, data = false) {
        ...
        if (task.isPending() && !data) {
            return ((_a = this.options) === null || _a === void 0 ? void 0 : _a.lazy) || this.getSelfOrParentOption(task, 'showSubtasks') !== false && task.hasSubtasks() && !task.subtasks.every((subtask) => !subtask.hasTitle())
                ? colorette_1.default.yellow(this.figures.pointer)
                : colorette_1.default.yellowBright(this.spinner[this.spinnerPosition]);
        }
        else if (task.isCompleted() && !data) {
            return task.hasSubtasks() && task.subtasks.some((subtask) => subtask.hasFailed()) ? colorette_1.default.yellow(this.figures.warning) : colorette_1.default.green(this.figures.tick);
        }
        else if (task.isRetrying() && !data) {
            return ((_b = this.options) === null || _b === void 0 ? void 0 : _b.lazy) ? colorette_1.default.yellow(this.figures.warning) : colorette_1.default.yellow(this.spinner[this.spinnerPosition]);
        }
        else if (task.isRollingBack() && !data) {
            return ((_c = this.options) === null || _c === void 0 ? void 0 : _c.lazy) ? colorette_1.default.red(this.figures.warning) : colorette_1.default.red(this.spinner[this.spinnerPosition]);
        }
        else if (task.hasRolledBack() && !data) {
            return colorette_1.default.red(this.figures.arrowLeft);
        }
        else if (task.hasFailed() && !data) {
            return task.hasSubtasks() ? colorette_1.default.red(this.figures.pointer) : colorette_1.default.red(this.figures.cross);
        }
        else if (task.isSkipped() && !data && this.getSelfOrParentOption(task, 'collapseSkips') === false) {
            return colorette_1.default.yellow(this.figures.warning);
        }
        else if (task.isSkipped() && (data || this.getSelfOrParentOption(task, 'collapseSkips'))) {
            return colorette_1.default.yellow(this.figures.arrowDown);
        }
        return !data ? colorette_1.default.dim(this.figures.squareSmallFilled) : this.figures.pointerSmall;
    }
