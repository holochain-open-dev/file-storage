...
function loadLauncher(name) {
    ...
    try {
        ...
        if (error.code === 'MODULE_NOT_FOUND') {
            throw new Error(`You need to add ${pkg} as a dependency of your project to use the --${name} flag.`);
        }
        else {
            throw error;
        }
...
function puppeteerLauncher(browsers = ['chrome']) {
    for (const browser of browsers) {
        if (!puppeteerBrowsers.includes(browser)) {
            throw new Error(`Unknown puppeteer browser: ${browser}. ` +
...
function playwrightLauncher(browsers = ['chromium']) {
    for (const browser of browsers) {
        if (!playwrightBrowsers.includes(browser)) {
            throw new Error(`Unknown playwright browser: ${browser}. ` +
