...
const BufferedLogger_1 = require("../reporter/BufferedLogger");
...
class TestRunnerCli {
    ...
    async reportTestResult(testFile, forceReport = false) {
        ...
        const allFinished = sessionsForTestFile.every(s => s.status === test_runner_core_1.SESSION_STATUS.FINISHED);
        if (!allFinished) {
    ...
    logSelectFilesMenu() {
        ...
        for (const testFile of this.runner.testFiles) {
            ...
            if (sessions.every(t => t.status === test_runner_core_1.SESSION_STATUS.FINISHED && !t.passed)) {
